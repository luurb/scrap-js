...
class Main extends IDbInit
{
    ...
    public function printResults()
    {
        ...
        $scraper = new ScraperResults();
        $db_connect = new GameDbConnect();
        ...
        foreach ($tr_children_list as $game) {
            $values = $scraper->getValues($game, $db_obj);
            $var = explode("[", $game->item(4)->nodeValue);
            $teams = $var[0];
            $bet = $game->item(6)->nodeValue;
            $db_connect->dbCheck($values, $teams, $bet, $db_obj);
        }
